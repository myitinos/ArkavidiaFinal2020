 ...
 *  Created by Arkavidia CTF and Admin Team
 */
...
int hex2int(char hex) {
  ...
  if (hex >= 'a' && hex <= 'f')
    return 0xa + hex - 'a';
  puts("Invalid hex char!");
  exit(EXIT_FAILURE);
}

int main(int argc, const char *argv[]) {
  if (argc < 2) {
    puts("Usage: pow <6 hex characters>\n");
    return 1;
  }
  const char *req = argv[1];
  byte digest_req[3];
  for (int i = 0; i < 3; ++i) {
    digest_req[i] = (hex2int(req[2*i]) << 4) | hex2int(req[2*i+1]);
  }
  printf("req: %02x%02x%02x\n", digest_req[0], digest_req[1], digest_req[2]);
  srand(time(NULL));
  char nonce[64];
  nonce[NONCE_LEN] = 0;
  ...
  for (;;) {
    //printf("try: %s\n", nonce);
    byte digest[16];
    md5((byte *)nonce, NONCE_LEN, digest, nil); // pads the input
    nonce[NONCE_LEN] = 0; // put back null terminator
    //printf("digest: %02x%02x%02x\n", digest[0], digest[1], digest[2]);
    char match = 1;
    for (int i = 0; i < 3; ++i) {
      if (digest[i] != digest_req[i]) {
        match = 0;
        break;
      }
    }
